...
export default {
  ...
  mounted () {
    ...
    const data = { 'email': 'eduardo@gmail.com', 'password': 'senhalegal', 'topic': 'krfind/1' }
    ...
    this.socket.on('message', (data) => {
      this.cars[0].position = [parseFloat(data['latitude']), parseFloat(data['longitude'])]
      let markersBounds = this.cars.map(car => car.position)
